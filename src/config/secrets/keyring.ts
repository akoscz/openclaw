...
function createMacOSProvider(
  ...
  async function ensureUnlocked(): Promise<void> {
    ...
    try {
      // Pass the password via an environment variable to avoid exposing it in
      // process arguments (visible in `ps aux` / `ps -e`). On macOS, environment
      // variables are not shown in the process list, making env-var delivery safe
      // for secret values. `security unlock-keychain` does NOT read from stdin
      // without -p — it either shows a GUI dialog or hangs in headless contexts.
      // Using a shell wrapper with an env-var is the standard macOS pattern for
      // secret delivery to child processes without process-arg exposure.
      const UNLOCK_TIMEOUT_MS = 10_000;
      await new Promise<void>((resolve, reject) => {
        const child = spawn(
          "sh",
          ["-c", 'security unlock-keychain -p "$KEYCHAIN_PASS" "$KEYCHAIN_PATH"'],
          {
            env: {
              ...process.env,
              KEYCHAIN_PASS: keychainPassword,
